...
from .filters import ListingFilter, SchemeFilter
from .models import (
    Schemes,
    Tags,
    Category,
...
class SchemeListView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["end_date"] = Schemes.closeDate
        print(Schemes.closeDate)
        return context
...
class MinistryList(ListView):
    ...
    def get_queryset(self):
        search_input = self.request.GET.get("search-query") or ""
        print(search_input)
        return Ministry.minsitryManager.queryset().filter(
...
class MinistryView(ListView):
    ...
    def get_queryset(self):
        ...
        posts = Schemes.schemeManager.active().filter(
            ministry__slug=self.kwargs["slug"]
        )
        print(posts)
        return posts
...
class StateView(FilterView, ListView):
    model = Schemes
    template_name = "schemes/schemes_list.html"
    context_object_name = "schemes"
    paginate_by = 8
    ...
    def get_context_data(self, **kwargs):
        ...
        context["filter"] = self.filterset  # Add the filter to context
        context["has_filter"] = any(self.request.GET.values())  # For clear filters link
        return context

    def get_queryset(self):
        self.tag = get_object_or_404(State, name=self.kwargs["name"])
        queryset = Schemes.schemeManager.active().filter(
            state__name=self.kwargs["name"]
        )
        return queryset
